...
# : 여러개의 작업(명령어)가 한 번에 처리되어야 할 때 사용
# - 중간에 하나라도 실패하면 전체 작업을 취소(Rollback)
# - 모든 작업이 문제없이 완료되면 확정(Commit)
/*
	은행에서 a가 b에게 돈을 보낼때
    a에서 출금 + b계좌에 입금 >> 두 작업은 반드시 함께 처리
    >> 하나만 성공하고 하나는 실패하면 돈이 사라지거나 두번 생기는 문제 발생
*/

### 2. 트랜잭션의 특징(ACID)
# Atomicity(원자성) : 모두 성공하거나 모두 실패(all or nothing)
# Consistency(일관성): 실행 전과 실행 후, 데이터의 일관성이 유지(DB의 상태 변화 X)
# Isolation(독립성): 동시에 여러 개의 트랜잭션이 실행되어도 서로 간섭하지 말아야 함
...
# 4) 실패하면 롤백: 작업 중 오류 발생 | 작업을 취소하고 싶을 때 (ROOLBACK)
#	>> 트랜잭션 시작 전 상태로 되돌림
...
select @@autocommit; # 1: 자동 커밋 활성화된 상태
...
### 스토어드 프로시저 작성: SQL 내의 조건문이나 반복문은 SQL스크립트 내에서 바로 실행X, 프로시저나 트리거 내에서 사용
delimiter $$
