...
#	ex) 기본키가 (A, B)(결합키)일 때, C가 A나 B 하나만으로 결정되지 않고 A,B 모두 알아야 C가 정해진다면 완전 종속 
...
### 2NF(제2정규화) 과정
# 1. 1NF(제1정규화)를 만족: 컬럼에 중복 데이터 없이, 원자값으로 구성
...
create table wrong_2nf(
    ...
    department varchar(50),-- 직원이 속한 부서
    location varchar(50),-- 부서 위치(2NF위반: department만으로 결정)
