...
# cf) create index로 생성된 인덱스는 drop table로 제거
# 	>> 기본키(PK), 고유키(uique)로 자동 생성된 인덱스는 drop index로 제거 X
#		: alter table 문으로 기본키나 고유키를 재거 > 자동 인덱스 제거
...
# : 인덱스 적용 상태 확인
# 1)Data_length(클러스터형 인덱스 크기):16384byte(16kb)
