...
# A is not null
# : null인 경우 false 반환, 아닌 경우 true 반환
select * from `members`
where
	points is null;
...
select * from `members`
where
	name like '_u%'; # 1개의 임의 문자 + u + 상관 없음: 이름의 2번째 글자가 u인 모든 회원 조회

select * from `members`
where
	name like '____'; # 이름이 4글자인 회원 조회
